<%@ page language="java" contentType="text/html; charset=utf-8"
 ...
 <%
   ...
   String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path;  
	...
	String phone= "",deSrc= "",sex= "",job= "" ,city = "", addr = "";
	String name = "",school="",profession="", deAddress ="" ;
	if(user.getUser()==null){
		response.sendRedirect("../login/default.jsp");
			...
			name = tea.getName();
			phone = tea.getPhone();
			school = tea.getSchool();
			sex = tea.getSex();
			profession = tea.getProfession();
			deAddress = tea.getDetailedAddress();
			job = tea.getJob();
			city = tea.getCity();
			addr = tea.getAddress();
       ...
       <div class="com-header-area" id="js-com-header-area">
		...
		<div class="page-role">
			...
			<form class="pxui-form-info" id="reg_form" name="reg_form" method="post" action="dtRegisterPhone" enctype="multipart/form-data" onSubmit="return checkRegForm()">
				...
				<div>
					<span>姓名：</span><p>
		            <input type="text" id="textfield" name="teacher.name" value=<%=name==null?"":name %> /></p>
				</div>
				<div id="sex">
					<span>性别：</span><p>
					<%if(sex==null||sex.equals("男")){ %>
					  ...
					  <%}else {%>
					  <input type="checkbox" class="radio"  value="男" >男&nbsp;&nbsp;&nbsp;
					  <input type="checkbox" class="radio"  value="女" checked="checked">女</p>
					  <input type="hidden" name="teacher.sex" value="女">
					  <%} %>
				</div>
				<% if(job==null||job.equals("本科生")){%>
				<input type="hidden" name="teacher.job" value="本科生">
				<div id="job">
					<span>职业：</span><p>
		            <input type="button" class="active" onclick="inp()" value="本科生" />
		            <input type="button" onclick="inp()" value="研究生" />
		            <input type="button" onclick="inp1()" value="教师"  /></p>
		            <div id="zzxs_" style="display:block;">
		            	<span>所在学校：</span>
		                <input placeholder="如：北京交通大学" type="text" name="teacher.school" value="<%=school==null?"":school %>"/><br>
		                <span>所在专业：</span>
		                <input placeholder="如：计算机科学" type="text" name="teacher.profession" value="<%=profession==null?"":profession %>"/>                      
		            </div>
				</div>
				<div id="pos" style="display:none;" >
					...
					<div>
						<span>地址：</span><p>
						<input class="tekang" type="text" name="teacher.detailedAddress" id="det_address"  value="<%=deAddress==null?"":deAddress %>"/></p>
					</div>
				</div>
				 <%}else if(job.equals("在职教师")){%>
				<input type="hidden" name="teacher.job" value="在职教师">
				<div id="job">
					<span>职业：</span><p>
		            <input type="button"  onclick="inp()" value="在校学生" />
		            <input type="button" class="active" onclick="inp1()" value="在职教师"  /></p>
		            <div id="zzxs_" style="display:block;">
		            	<span>所在学校：</span>
		                <input placeholder="如：北京交通大学" type="text" name="teacher.school" value="<%=school==null?"":school %>"/><br>
		                <span>所教课程：</span>
				...
				<div id="pos" style="display:block;" >
					<div >
						<span>地区：</span><p>
						<select id="selProvince" name="teacher.city" onchange="provinceChange();" value=<%=city==null?"":city %>>
						</select>&nbsp;&nbsp;
						<select id="selCity" name="teacher.address" value=<%=addr==null?"":addr %>></select>
						<!--<i id='position'></i>
						<input readOnly="readonly" class="tekang" type="hidden" name="teacher.address"  />  --></p>
					</div>
					<div>
				...
				<%}else {%>
				<input type="hidden" name="teacher.job" value="研究生">
				<div id="job">
					<span>职业：</span><p>
		            <input type="button"  onclick="inp()" value="本科生" />
		            <input type="button" class="active" onclick="inp()" value="研究生" />
		            <input type="button" onclick="inp1()" value="教师"  /></p>
		            <div id="zzxs_" style="display:block;">
		            	<span>所在学校：</span>
		                <input placeholder="如：北京交通大学" type="text" name="teacher.school" value="<%=school==null?"":school %>"/><br>
				...
				<div>
					<span>个人简介：</span><p>
					<textarea rows="3" cols="20" placeholder="个人简介"   name="teacher.introduction" id="textarea1" >${tea.introduction }</textarea>
					</p>
				</div>
				<div>
					<span>详细介绍：</span>
					<p style="background-color:white;">
						<textarea style="height: 100px;" cols="50" name="teacher.detailedIntroduction" id="textarea2" placeholder="详细介绍：如某年某月某日，得到什么奖项">${tea.detailedIntroduction }</textarea>
					</p>
				</div>
				<hr>
				<h3>结账信息<em>(选填)</em></h3>
				<div>
					<span>银行账户：</span><p>
					<input  placeholder="请填入银行账户，以便将您的收入打入该账户" name="teacher.cardNo" id="bankCard" value="${tea.cardNo}" type="text"/></p>
				</div>
				<div>
					<span>开户银行：</span><p>
					<input  placeholder="开户银行地址" name="teacher.cardType"  value="${tea.cardType }" type="text"/></p>
				</div>
				<input type="hidden" name="teacher.lng" id="lng">
				<input type="hidden" name="teacher.lat" id="lat">
