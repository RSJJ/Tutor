<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />

<title></title>
<link href="css/global.css" rel="stylesheet" type="text/css"  charset="utf-8"  />
<link href="css/erji.css" rel="stylesheet" type="text/css"  charset="utf-8"  />
<script src="js/jquery.min.js" type="text/javascript" charset="utf-8" language="javascript"></script>
<script src="js/formCheck.js" type="text/javascript" charset="utf-8" language="javascript"></script>
<script src="js/jquery.fancybox-1.3.4.js" type="text/javascript" charset="utf-8" language="javascript"></script>
<script src="js/t_team_js.js" type="text/javascript" charset="utf-8" language="javascript"></script>
<!-- 首页轮播图 start --> 


<!--[if IE 6]>
<script src="js/background.png.js" type="text/javascript" charset="utf-8" language="javascript"></script>
<script type="text/javascript">DD_belatedPNG.fix('*');</script>
<![endif]--> 

</head>

<body>

<div class="container">
<!--导航 start-->
   <div class="erji_top">
      <div class="content">
    	<jsp:include page="s_header.jsp"></jsp:include>
      </div>
    </div>
<!--导航 stop-->   

<!--微信 start--> 
<div class="floatDiv">
    <div class="floatDivOne">
      <div class="theFirst"><img src="images/zxzx.png" /></div>
      <div class="theLast"> <img onclick="NTKF.im_openInPageChat()" src="images/intalker.jpg" /></div>
    </div>
    <div class="floatDivTwo">
      <div class="theFirstTwo"><img src="images/wexin_4.png" /></div>
      <div class="theLastTwo"><img src="images/weixin_2.png" /></div>
    </div>
  </div>
<!--微信 stop-->   

    <div class="main" style="background:#edeff0">
    <div class="content">
    
    <div id="main">

        <div class="container" style="background:none;">


        <div class="course-sidebar">

                <div class="course-sidebar-type">
    
              <ul   id="suckertree1">
                        <li class="course-category curr"><a class="course-category-link" href="#" data-id="0">全部</a></li>
                        
                         <li  class="course-category course-category-more">
                             <a  class="course-category-link" href="#" data-id="1">小学课程</a>
                             <div class="course-category-sort">
                                    <div class="course-category-sort-bg"></div>
                                    <ul>
                                    	<li><h3>小一：</h3></li>
                                  		<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                </ul>
                                <ul>
                                    	<li><h3>小二：</h3></li>
                                  		<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                  		<li><a href="#">体育类</a></li>
                                  		<li></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                </ul>
                                <ul>
                                    	<li><h3>小三：</h3></li>
                                    	<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                  		<li><a href="#">体育类</a></li>
                                  		<li></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                  		<li><a href="#">科普类</a></li>
                                </ul>
                                  <ul>
                                    	<li><h3>小四：</h3></li>
                                    	<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                  		<li><a href="#">体育类</a></li>
                                  		<li></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                  		<li><a href="#">科普类</a></li>
                                </ul>
                                  <ul>
                                    	<li><h3>小五：</h3></li>
                                    	<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                  		<li><a href="#">体育类</a></li>
                                  		<li></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                  		<li><a href="#">科普类</a></li>
                                </ul>
                                  <ul>
                                    	<li><h3>小六：</h3></li>
                                    	<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                 	 	<li><a href="#">微&nbsp;机</a></li>
                                 	 	<li></li>
                                  		<li><a href="#">体育类</a></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                  		<li><a href="#">科普类</a></li>
                                  		
                                </ul>
                             </div>
                         </li>
                         <li  class="course-category course-category-more">
                             <a  class="course-category-link" href="#" data-id="2">初中课程</a>
                             <div class="course-category-sort">
                                <div class="course-category-sort-bg"></div>
                                     <ul>
                                    	<li><h3>初一：</h3></li>
                                    	<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                  		<li><a href="#">微&nbsp;机</a></li>
                                  		<li></li>
                                  		<li><a href="#">政&nbsp;治</a></li>
                                  		<li><a href="#">历&nbsp;史</a></li>
                                 	 	<li><a href="#">地&nbsp;理</a></li>
                                  		<li><a href="#">生&nbsp;物</a></li>
                                  		<li></li>
                                  		<li><a href="#">体育类</a></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                </ul>
                                  <ul>
                                    	<li><h3>初二：</h3></li>
                                    	<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                  		<li><a href="#">微&nbsp;机</a></li>
                                  		<li></li>
                                  		<li><a href="#">政&nbsp;治</a></li>
                                  		<li><a href="#">历&nbsp;史</a></li>
                                 	 	<li><a href="#">地&nbsp;理</a></li>
                                  		<li><a href="#">生&nbsp;物</a></li>
                                  		<li></li>
                                  		<li><a href="#">物&nbsp;理</a></li>
                                  		<li><a href="#">化&nbsp;学</a></li>
                                  		<li><a href="#">体育类</a></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                </ul>
                                  <ul>
                                    	<li><h3>初三：</h3></li>
                                    	<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                  		<li><a href="#">微&nbsp;机</a></li>
                                  		<li></li>
                                  		<li><a href="#">政&nbsp;治</a></li>
                                  		<li><a href="#">历&nbsp;史</a></li>
                                  		<li><a href="#">生&nbsp;物</a></li>
                                  		<li><a href="#">体育类</a></li>
                                  		<li></li>
                                  		<li><a href="#">物&nbsp;理</a></li>
                                  		<li><a href="#">化&nbsp;学</a></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                  		
                                </ul>
                            </div>
                         </li>
                         <li  class="course-category course-category-more">
                            <a  class="course-category-link" href="#" data-id="3">高中课程</a>
                            <div class="course-category-sort">
                                <div class="course-category-sort-bg"></div>
                                <ul>
                                    	<li><h3>高一：</h3></li>
                                    	<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                  		<li><a href="#">微&nbsp;机</a></li>
                                  		<li></li>
                                  		<li><a href="#">政&nbsp;治</a></li>
                                  		<li><a href="#">历&nbsp;史</a></li>
                                 	 	<li><a href="#">地&nbsp;理</a></li>
                                  		<li><a href="#">生&nbsp;物</a></li>
                                  		<li></li>
                                  		<li><a href="#">物&nbsp;理</a></li>
                                  		<li><a href="#">化&nbsp;学</a></li>
                                  		<li><a href="#">体育类</a></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                </ul>
                                  <ul>
                                    	<li><h3>高二：</h3></li>
                                    	<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                  		<li><a href="#">微&nbsp;机</a></li>
                                  		<li></li>
                                  		<li><a href="#">政&nbsp;治</a></li>
                                  		<li><a href="#">历&nbsp;史</a></li>
                                 	 	<li><a href="#">地&nbsp;理</a></li>
                                 	 	<li><a href="#">文&nbsp;综</a></li>
                                  		<li></li>
                                  		<li><a href="#">物&nbsp;理</a></li>
                                  		<li><a href="#">化&nbsp;学</a></li>
                                  		<li><a href="#">生&nbsp;物</a></li>
                                  		<li><a href="#">理&nbsp;综</a></li>
                                  		<li></li>
                                  		<li><a href="#">体育类</a></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                </ul>
                                  <ul>
                                    	<li><h3>高三：</h3></li>
                                    	<li><a href="#">语&nbsp;文</a></li>
                                  		<li><a href="#">数&nbsp;学</a></li>
                                 	 	<li><a href="#">英&nbsp;语</a></li>
                                  		<li><a href="#">微&nbsp;机</a></li>
                                  		<li></li>
                                  		<li><a href="#">政&nbsp;治</a></li>
                                  		<li><a href="#">历&nbsp;史</a></li>
                                 	 	<li><a href="#">地&nbsp;理</a></li>
                                 	 	<li><a href="#">文&nbsp;综</a></li>
                                  		<li></li>
                                  		<li><a href="#">物&nbsp;理</a></li>
                                  		<li><a href="#">化&nbsp;学</a></li>
                                  		<li><a href="#">生&nbsp;物</a></li>
                                  		<li><a href="#">理&nbsp;综</a></li>
                                  		<li></li>
                                  		<li><a href="#">体育类</a></li>
                                 	 	<li><a href="#">艺术类</a></li>
                                  		
                                </ul>
                            </div>
                         </li>
                         <li  class="course-category course-category-more">
                            <a  class="course-category-link" href="#" data-id="4">考研课程</a>
                            <div class="course-category-sort">
                                <div class="course-category-sort-bg"></div>
                                <ul>
                                  <li><a href="#">C++</a></li>
                                  <li><a href="#">Go </a></li>
                                </ul>
                            </div>
                         </li>
                       </ul>
                    <div class="dot-sidebar-curr" style="top:0px"></div>
                </div>
    
            </div>
            <div class="course-content">
                <div class="course-tools">
                    <h2>全部</h2>
                    <ul class="course-sidebar-sort">
                        <li></li>
                    </ul>
                </div>
                <div class="course-list">
                    <div class="js-course-list">
                        <div class="ul_6tips">
                        <ul>
                        <li>
                                <div class="erji_rtx"><img src="images/123.jpg" /></div>
                                <div class="fl erji_rtx1">李宏伟</div><div class="fr erji_rtx1">本科</div>
      <div class="ul_6txt">
                                    <p><font class="f_18">Ernie<br /></font>——汽车开发工程师</p>
                                    <p>我喜欢通过经典英语的教材和老师进行话题讨论，有助进行话题讨论，有助于我组织语言能于我组织语言能进行话题讨论，有助于我组织语言能力的提高。老师也非常有耐常沟通了，还会继续努力！</p>
                                <div class="fl erji_rtx2"><a href="#">授课视频</a></div><div class="fr erji_rtx2"><a href="#">预约课程</a></div>    
                                </div>
                        </li>
                        
                        <li>
                                <div class="erji_rtx"><img src="images/123.jpg" /></div>
                                <div class="fl erji_rtx1">李宏伟</div><div class="fr erji_rtx1">本科</div>
      <div class="ul_6txt">
                                    <p><font class="f_18">Ernie<br /></font>——汽车开发工程师</p>
                                    <p>我喜欢通过经典英语的教材和老师进行话题讨论，有助进行话题讨论，有助于我组织语言能于我组织语言能进行话题讨论，有助于我组织语言能力的提高。老师也非常有耐常沟通了，还会继续努力！</p>
                                <div class="fl erji_rtx2"><a href="#">授课视频</a></div><div class="fr erji_rtx2"><a href="#">预约课程</a></div>    
                                </div>
                        </li>
                        
                        <li>
                                <div class="erji_rtx"><img src="images/123.jpg" /></div>
                                <div class="fl erji_rtx1">李宏伟</div><div class="fr erji_rtx1">本科</div>
      <div class="ul_6txt">
                                    <p><font class="f_18">Ernie<br /></font>——汽车开发工程师</p>
                                    <p>我喜欢通过经典英语的教材和老师进行话题讨论，有助进行话题讨论，有助于我组织语言能于我组织语言能进行话题讨论，有助于我组织语言能力的提高。老师也非常有耐常沟通了，还会继续努力！</p>
                                <div class="fl erji_rtx2"><a href="#">授课视频</a></div><div class="fr erji_rtx2"><a href="#">预约课程</a></div>    
                                </div>
                        </li>
                        
                        <li>
                                <div class="erji_rtx"><img src="images/123.jpg" /></div>
                                <div class="fl erji_rtx1">李宏伟</div><div class="fr erji_rtx1">本科</div>
      <div class="ul_6txt">
                                    <p><font class="f_18">Ernie<br /></font>——汽车开发工程师</p>
                                    <p>我喜欢通过经典英语的教材和老师进行话题讨论，有助进行话题讨论，有助于我组织语言能于我组织语言能进行话题讨论，有助于我组织语言能力的提高。老师也非常有耐常沟通了，还会继续努力！</p>
                                <div class="fl erji_rtx2"><a href="#">授课视频</a></div><div class="fr erji_rtx2"><a href="#">预约课程</a></div>    
                                </div>
                        </li>
                        
                        <li>
                                <div class="erji_rtx"><img src="images/123.jpg" /></div>
                                <div class="fl erji_rtx1">李宏伟</div><div class="fr erji_rtx1">本科</div>
      <div class="ul_6txt">
                                    <p><font class="f_18">Ernie<br /></font>——汽车开发工程师</p>
                                    <p>我喜欢通过经典英语的教材和老师进行话题讨论，有助进行话题讨论，有助于我组织语言能于我组织语言能进行话题讨论，有助于我组织语言能力的提高。老师也非常有耐常沟通了，还会继续努力！</p>
                                <div class="fl erji_rtx2"><a href="#">授课视频</a></div><div class="fr erji_rtx2"><a href="#">预约课程</a></div>    
                                </div>
                        </li>
                        
                        <li>
                                <div class="erji_rtx"><img src="images/123.jpg" /></div>
                                <div class="fl erji_rtx1">李宏伟</div><div class="fr erji_rtx1">本科</div>
      <div class="ul_6txt">
                                    <p><font class="f_18">Ernie<br /></font>——汽车开发工程师</p>
                                    <p>我喜欢通过经典英语的教材和老师进行话题讨论，有助进行话题讨论，有助于我组织语言能于我组织语言能进行话题讨论，有助于我组织语言能力的提高。老师也非常有耐常沟通了，还会继续努力！</p>
                                <div class="fl erji_rtx2"><a href="#">授课视频</a></div><div class="fr erji_rtx2"><a href="#">预约课程</a></div>    
                                </div>
                        </li>
                        
                        <li>
                                <div class="erji_rtx"><img src="images/123.jpg" /></div>
                                <div class="fl erji_rtx1">李宏伟</div><div class="fr erji_rtx1">本科</div>
      <div class="ul_6txt">
                                    <p><font class="f_18">Ernie<br /></font>——汽车开发工程师</p>
                                    <p>我喜欢通过经典英语的教材和老师进行话题讨论，有助进行话题讨论，有助于我组织语言能于我组织语言能进行话题讨论，有助于我组织语言能力的提高。老师也非常有耐常沟通了，还会继续努力！</p>
                                <div class="fl erji_rtx2"><a href="#">授课视频</a></div><div class="fr erji_rtx2"><a href="#">预约课程</a></div>    
                                </div>
                        </li>
                        
                        <li>
                                <div class="erji_rtx"><img src="images/123.jpg" /></div>
                                <div class="fl erji_rtx1">李宏伟</div><div class="fr erji_rtx1">本科</div>
      <div class="ul_6txt">
                                    <p><font class="f_18">Ernie<br /></font>——汽车开发工程师</p>
                                    <p>我喜欢通过经典英语的教材和老师进行话题讨论，有助进行话题讨论，有助于我组织语言能于我组织语言能进行话题讨论，有助于我组织语言能力的提高。老师也非常有耐常沟通了，还会继续努力！</p>
                                <div class="fl erji_rtx2"><a href="#">授课视频</a></div><div class="fr erji_rtx2"><a href="#">预约课程</a></div>    
                                </div>
                        </li>
                        
                        <li>
                                <div class="erji_rtx"><img src="images/123.jpg" /></div>
                                <div class="fl erji_rtx1">李宏伟</div><div class="fr erji_rtx1">本科</div>
      <div class="ul_6txt">
                                    <p><font class="f_18">Ernie<br /></font>——汽车开发工程师</p>
                                    <p>我喜欢通过经典英语的教材和老师进行话题讨论，有助进行话题讨论，有助于我组织语言能于我组织语言能进行话题讨论，有助于我组织语言能力的提高。老师也非常有耐常沟通了，还会继续努力！</p>
                                <div class="fl erji_rtx2"><a href="#">授课视频</a></div><div class="fr erji_rtx2"><a href="#">预约课程</a></div>    
                                </div>
                        </li>
                        </ul>
                    </div>
                      
                    </div>
                </div>
            </div>
        </div>
    
    </div>
    
    </div>
  </div>
  
  
  
  
<!--底部 start-->   
<jsp:include page="s_bottom.jsp"></jsp:include>
<!--底部 stop-->     
</div>

</body>
</html>